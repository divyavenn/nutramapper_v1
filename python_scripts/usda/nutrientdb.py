...
class NutrientDB:
	def __init__(self, database_name='usda.db'):
		...
		self.create_table_stmt["nut_data"] = '''DROP TABLE IF EXISTS nut_data; CREATE TABLE nut_data 
									(NDB_No text, Nutr_No, Nutr_Val real, Num_Data_Pts integer, Std_Error real,
									 Src_Cd, Deriv_Cd, Ref_NDB_No, Add_Nutr_Mark, Num_Studies integer,
										Min real, Max real, DF integer, Low_EB real, Up_EB real, Stat_cmt, AddMod_Date);
	...
	def refresh(self, filename, datatype):
		...
		with open(filename, 'rU') as f:
			for line in f:
				...
				fields = [unicode(field.strip().strip('~').replace('"', '').replace("'", "").replace("(Includes foods for USDA's Food Distribution Program)", "")) for field in line.split('^')]
				#print(fields)
...
def main():
	...
	parser.add_argument('-db', '--database', dest='database', help='The name of the SQLite file to read/write nutrient info. (default: usda.db)', default='usda.db')
